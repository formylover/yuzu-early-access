...
void UnswizzleSubrect(u32 line_length_in, u32 line_count, u32 pitch, u32 width, u32 bytes_per_pixel,
    ...
    for (u32 line = 0; line < line_count; ++line) {
        const u32 dst_y = line + origin_y;
        const auto& table = LEGACY_SWIZZLE_TABLE[dst_y % GOB_SIZE_Y];

        const u32 block_y = dst_y >> GOB_SIZE_Y_SHIFT;
        const u32 dst_offset_y = (block_y >> block_height) * block_size +
                                 ((block_y & block_height_mask) << GOB_SIZE_SHIFT);
        for (u32 column = 0; column < line_length_in; ++column) {
            const u32 dst_x = column + origin_x;
            const u32 offset_x = (dst_x >> GOB_SIZE_X_SHIFT) << x_shift;

            const u32 swizzled_offset = dst_offset_y + offset_x + table[dst_x % GOB_SIZE_X];
